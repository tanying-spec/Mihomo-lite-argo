# ✨ Mihomo Lite - 一键配置脚本 V1.12.4（Argo 稳定版）
<!-- GitHub Badges -->
...
安装器默认始终从 GitHub 下载并校验 `mh.sh`，不会静默使用当前目录中的同名文件。仅在开发测试时可通过 `MH_LOCAL_SCRIPT=/path/to/mh.sh sh install.sh` 明确安装本地脚本。
...
* **🏠 首页仪表盘**：打开 `mh` 即可查看 Mihomo、Tunnel、边缘连接、节点数量、内存余量、OOM Kill 和当前 DNS；仅读取本地状态，不执行耗时公网探测。
* **🔗 节点生成**：一键生成代理节点，并自动输出可复制导入的节点链接。
* **📊 节点管理**：查看所有已建节点、单节点链接以及 **Base64 聚合订阅**，支持节点名称、监听端口、凭据和 WebSocket 模式编辑，也支持单节点删除、一键清空和批量重命名。
* **✅ 节点验证**：创建节点后自动检查配置、服务、TCP/UDP 监听和端口归属；WebSocket 节点继续验证本地握手，并在条件具备时检测 Cloudflare CDN/Tunnel 公网路由。也可在节点管理中随时复测。
* **🧾 脱敏诊断**：执行 `mh report` 或使用主菜单 12，可生成适合反馈故障的诊断报告；自动隐藏公网 IP、域名、节点名称、端口明细、凭据、Token、证书路径和节点链接。
* **⚙️ 服务运维**：一键查看 YAML 配置文件、重启服务进程。
* **🚀 性能优化**：支持运行时参数调优、省资源稳连模式、高吞吐/跑满带宽模式、网络状态检测与一键安全优化，以及公网 IP 本地缓存。
...
* **🛡️ 稳定与回滚**：配置写入及服务重启前执行 Mihomo 自检，OpenRC/Systemd 启动后验证真实进程和内部端口，失败自动恢复旧配置；内核、cloudflared 和管理脚本均可回滚上一版本。
...
* **🪶 低开销守护**：健康守护优先使用 `nc` 完成一次性 WebSocket 握手和本机 metrics 查询，收到结果立即退出；缺少 `nc` 时自动回退到 `curl`，避免每分钟产生持续数秒的无效等待。
* **🔄 无缝升级**：支持一键更新 Mihomo、cloudflared 与管理脚本，菜单 `10` 提供版本回滚。
* **🩺 健康检查**：菜单 `11` 或命令 `mh doctor` 检查配置、服务、数据库、监听端口、DNS 上游、公共 UDP/TCP 53、容器内存余量、OOM Kill 增量、Tunnel 连接和 Token 权限，并明确区分节点监听故障与 DNS 上游不可达。
...
三种预设会保持明确差异：省资源模式使用更低的内存目标和更高的 `GOGC`，系统推荐模式取折中值，高吞吐模式使用更高的内存目标和更积极的 GC。单核或 CPU 配额受限的容器中，三档的 `GOMAXPROCS` 都可能是 `1`，这是正常现象。内存检测会取 cgroup 限制与 `/proc/meminfo` 可见内存中的较小值；如果两者都无法读取，菜单会显示警告并采用保守兜底值。

菜单输入 `55` 可打开简化的网络诊断菜单：查看当前网络状态、一键完成约 25 MB 的真实测试并应用经过验证的安全建议、查看已应用参数，以及恢复修改前参数。测试会显示吞吐、首字节及 TCP 重传、超时、UDP 缓冲错误、监听丢弃和 OOM 增量；测试失败时不修改任何参数。
...
* 默认关闭 `fake-ip` 缓存，DNS 使用 `redir-host`。生成配置时会优先验证并使用 `/etc/resolv.conf` 中的系统 DNS；只有公共 DNS 实测可用时才会作为回退，不再写死 `1.1.1.1/8.8.8.8`。
* Mihomo 每次重启前都会验证当前 DNS 上游；不可达时自动选择有效上游、重写配置并重启复验一次。`mh update` 会自动迁移旧版生成的错误 DNS 配置，也可手动执行 `mh dns-migrate`。
...
* 菜单 `55` 采用诊断优先的保守网络优化；容器无权限的 sysctl 项不会伪报成功，首次修改前记录原值，完整卸载时恢复，并会清理旧版生成的激进持久化配置。
