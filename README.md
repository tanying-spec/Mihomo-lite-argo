# ✨ Mihomo Lite - 一键配置脚本 V1.12.3（Argo 稳定版）
<!-- GitHub Badges -->
...
三种预设会保持明确差异：省资源模式使用更低的内存目标和更高的 `GOGC`，系统推荐模式取折中值，高吞吐模式使用更高的内存目标和更积极的 GC。单核或 CPU 配额受限的容器中，三档的 `GOMAXPROCS` 都可能是 `1`，这是正常现象；如果无法读取可信的 cgroup 内存上限，菜单会显示警告并采用保守兜底值。
