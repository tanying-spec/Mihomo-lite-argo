...
菜单输入 `55` 可执行无损网络诊断与优化：显示内存限制、拥塞控制、qdisc、TCP 重传、UDP 缓冲错误和监听丢弃；仅在内核支持时启用 BBR，并确保 TCP 窗口缩放开启。脚本不会自动放大 TCP/UDP 缓冲、连接队列或修改 TIME_WAIT、Keepalive、端口范围；容器不可见的参数明确标记为由宿主机管理。
